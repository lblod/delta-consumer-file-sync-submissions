...
export async function getUrisToDelete(oldUris = []){
  let results = [];

  //TODO: some attempt at being economical in lines of code, might be
  // subject to improvement
  const queryStr = subjectBind => {
    return `
      ${PREFIXES}
  ...
  if(!oldUris.length){
    results = (parseResult(await query(queryStr('')))).map(t => t.pFile);
  }
  else {
    //We query one by one, since else it might be heavy on the database.
    for(const uri of oldUris){
      const subjectBind = `BIND(${sparqlEscapeUri(uri)} as ?oldPFile)`;
      results = [ ...(parseResult(await query(queryStr(subjectBind)))).map(t => t.pFile), ...results];
    }
  }
  return results;
}
...
export function calculateNewFileData(fileData){
  ...
  if(fileType == 'RegularFile' || fileType == 'CachedFile' ){
   ...
   newData.newPuri = `share://submissions/${newData.newPUuid}.${fileData.extension || 'ttl'}`; //TODO: fix the source data.
  }
  else {
